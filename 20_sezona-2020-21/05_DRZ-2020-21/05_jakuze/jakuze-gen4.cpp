...
int main()
{
	...
	fptc=fopen(ch,"w");
	fprintf(fptc,"%d %d %d\n",n,m,q);
	for(int i=0;i<=n;i++) for(int j=0;j<=m;j++) {v[i].push_back(0); a[i].push_back(0); vi[i].push_back(0);}
	pathgen(uniform_int_distribution<int>(1,n)(rng),uniform_int_distribution<int>(1,m)(rng));
	for(int i=1;i<=n;i++) for(int j=1;j<=m;j++) a[i]=v[i];
	for(int i=1;i<=m*n;i++) {sx[i]=ix[i]; sy[i]=iy[i];}
	...
	if(br<10) fprintf(fp,"\n0%d.in\t%d\t%d\t%d\t%d\t4",br,n,m,q,seed);
	else fprintf(fp,"\n%d.in\t%d\t%d\t%d\t%d\t4",br,n,m,q,seed);
	fclose(fp);
}
