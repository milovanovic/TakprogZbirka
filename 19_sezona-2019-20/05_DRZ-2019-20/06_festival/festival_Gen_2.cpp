...
int main()
{
	...
	for(int i=1;i<=2*n;i++)
	{
		if(!a[i])
		{
			...
			a[i]=per[t];
			int x=uniform_int_distribution<int>(1,d)(rng),p=i,lst=0;
			if(rnd) x=uniform_int_distribution<int>(d*d,2*d*d)(rng);
	...
	if(br<10) fprintf(fp,"\n0%d.in\t%d\t%d\t%d\t%d\t\t2",br,n,d,q,seed);
	else fprintf(fp,"\n%d.in\t%d\t%d\t%d\t%d\t\t2",br,n,d,q,seed);
	fclose(fp);
}
